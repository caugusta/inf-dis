...
def write_neighbourhood_epidemics_redo(susc, trans, inf_period, eps, repetitions, pop):
        ...
        while True:
                if trans < 50:
                        for rep in range(repetitions):
                                print 'neigh', trans, inf_period, ep, rep, count
    ...
    with open('parameters_neighbourhood_Dec23_redo.txt', 'w') as newfile1:
        np.savetxt(newfile1, paras, fmt = ['%f', '%f', '%f', '%f'])

    with open('epidemics_neighbourhood_Dec23_redo.json', 'w') as newfile2:
...
def write_neighbourhood_epidemics(susc, trans, inf_period, eps, repetitions, pop):
    ...
    print 'number of parameter rows', paras[:,0].shape
    with open('parameters_neighbourhood_Dec23.txt', 'w') as newfile1:
        np.savetxt(newfile1, paras, fmt = ['%f', '%f', '%f', '%f'])

    with open('epidemics_neighbourhood_Dec23.json', 'w') as newfile2:
        json.dump(count_list, newfile2)
...
def write_exponential_epidemics(susc, trans, inf_period, eps, repetitions, pop):
    ...
    print 'number of parameter rows', paras[:,0].shape
    with open('parameters_exponential_Dec23.txt', 'w') as newfile1:
        np.savetxt(newfile1, paras, fmt = ['%f', '%f', '%f', '%f'])


    with open('epidemics_exponential_Dec23.json', 'w') as newfile2:
        json.dump(count_list, newfile2)
...
def write_powerlaw_epidemics(susc, trans, inf_period, eps, repetitions, pop):    
    ...
    print 'number of parameter rows', paras[:,0].shape
    with open('parameters_powerlaw_Dec23.txt', 'w') as newfile1:
        np.savetxt(newfile1, paras, fmt = ['%f', '%f', '%f', '%f'])


    with open('epidemics_powerlaw_Dec23.json', 'w') as newfile2:
        json.dump(count_list, newfile2)
...
if __name__ == "__main__":
    ...
    neigh_trans = [1.0, 1.5, 2.0]
    #neigh_trans = [7.5, 3.5, 2.4]
    inf_period = [2, 3]
    eps = [0., 0.01, 0.02, 0.05] #was 0, 0.01, 0.02, 0.05
 #   mean_val = np.array([1., 1.])
 #   sigma_val = np.array([[1., 0.], [0., 1.]])
 #   full_mat = create_pop(mean_val, sigma_val, pop)
    reps = 400
    neigh_eps = [0.1, 0.2, 0.5, 0.7] 

#0.300000 2.000000 2.000000 0.100000
#0.300000 2.000000 2.000000 0.200000
   ...
   # write_powerlaw_epidemics(susc, trans, inf_period, eps, reps, pop)
 ...
 #   write_neighbourhood_epidemics(susc, neigh_trans, inf_period, neigh_eps, reps, pop) 
    write_neighbourhood_epidemics_redo(susc, redo_trans, redo_inf_period, redo_eps, reps, pop)
